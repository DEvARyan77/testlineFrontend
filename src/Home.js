...
function Home() {
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        if (timer === 0) {
          ...
          isTimerRunningRef.current = false // Timer stopped when it reaches zero
        }
      }, [timer]);
    ...
    const getRandomQuestionIndex = () => {
        ...
        if(usedQuestions.length===(data.questions_count-1)){
            clearInterval(timerIntervalRef.current);
            setStartingScreen(false)
            setRulesScreen(false)
            setResultScreen(true)
        ...
        // Check that the random index is not already used
        do {
            console.log("df")
            console.log(usedQuestions.length)
          randomIndex = Math.floor(Math.random() * data.questions.length);
        } while (usedQuestions.includes(randomIndex));
        setUsedQuestions(prevUsedQuestions => [...prevUsedQuestions, randomIndex]);
    ...
    return (
        <div>
            {
                (startingScreen && data)?(<StartingScreen heading={data.title} topic={data.topic} setStartingScreen={setStartingScreen} setRulesScreen={setRulesScreen}></StartingScreen>):null
            }
            {
                (rulesScreen && data)?(<Rules setQuestionScreen={setQuestionScreen} setRulesScreen={setRulesScreen}></Rules>):null
            }
            {
                (questionScreen && data)?(<Question question={data.questions[currentQuestion]} handleNext={handleNext} setMarks={setMarks} marks={marks} timer={timer}></Question>):null
            }
            {
                (resultScreen && data)?(<Result timer={timer} marks={marks}></Result>):null
            }
        </div>
    );
}
